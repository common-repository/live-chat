	...
	<div id="lc-chat" style="border: 1px solid #<?php echo get_option('live_chat_color2'); ?> !important;" >
        ...
        if(get_option('live_chat_only_members')=="1" and strlen($current_user->display_name)==0){
			...
			<div class="lc-chat-head" >
				<div id="lc-status-icon" class="lc-status-icon-offline"></div><?php if(get_option('live_chat_author_link')=="1"){echo '<a href="http://daext.com" target="_blank">Live Chat</a>';}else{echo '<p>Live Chat</p>';} ?>
			...
			<input type="hidden" name="livechatusername" id="livechatusername" value="<?php echo htmlspecialchars( lc_get_username() , ENT_QUOTES); ?>">
...
function lc_get_username(){
	...
	if(get_option('live_chat_only_members')=="1" and strlen($current_user->display_name)>0){
		...
		if(isset($_COOKIE["livechatusername"])){
			$lc_username=htmlspecialchars( $_COOKIE["livechatusername"] , ENT_QUOTES);
		}else{
			$lc_username="";
		}
	}
